...
test.describe('MainframeHub Web UI', () => {
    ...
    test('should validate new PR form', async ({ page }) => {
        ...
        const validationMessage = await promptInput.evaluate((el) => el.validationMessage);
    ...
    test('should handle API errors gracefully', async ({ page }) => {
        ...
        // Open setup modal
        await page.click('#btn-setup-pr');
        // Enter invalid PR number (0)
        await page.fill('#input-pr-number', '0');
        await page.click('button[type="submit"]');
...
test.describe('MainframeHub Web UI with Mock Session', () => {
    ...
    test.beforeEach(async () => {
        ...
        // Initialize git repo
        git.init(clonePath);
        git.config(clonePath, 'user.name', 'Test User');
        git.config(clonePath, 'user.email', 'test@example.com');
        // Create tmux session
        await tmux.create({
            id: testSessionId,
